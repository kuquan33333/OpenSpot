...
function formatTrack(track, previewURL) {
...
/**
 * Fetch SoundCloud's real trending chart for the Home feed. Search queries
 * remain the fallback for category sections such as remix and vinahouse.
 */
function fetchTrendingTracks(limit) {
  var boundedLimit = Number(limit) || 20;
  if (boundedLimit < 1) boundedLimit = 1;
  if (boundedLimit > 50) boundedLimit = 50;

  try {
    var data = scGet("charts", "kind=trending&genre=soundcloud:genres:all-music&limit=" + boundedLimit + "&offset=0");
    var collection = data.collection || data.data || [];
    var results = [];

    for (var i = 0; i < collection.length && results.length < boundedLimit; i++) {
      var item = collection[i] || {};
      var rawTrack = item.track || item;
      if (!rawTrack || !rawTrack.id) continue;
      trackCacheSet(rawTrack.id, rawTrack);
      var track = formatTrack(rawTrack);
      if (track) results.push(track);
    }

    log.info("[SC] Found", results.length, "trending tracks");
    return results;
  } catch (e) {
    log.warn("[SC] Trending chart failed, falling back to search:", e.message);
    return [];
  }
}
...
function download(trackID, quality, outputPath, onProgress, options) {
  ...
  var qualityParts = String(quality || "mp3_128").split("_");
...
/**
 * Resolve a SoundCloud progressive transcoding to a short-lived direct audio
 * URL. OpenSpot's provider bridge exposes this value through preview_url for
 * playback; the download provider continues to use its own file.download
 * path so an expiring URL is never persisted as download metadata.
 */
function resolveProgressiveStreamURL(trackData, preferFormat) {
  if (!trackData) return { url: "", format: null, error: "track data is missing" };

  var transcodings = (trackData.media && trackData.media.transcodings) || [];
  var trackAuth = trackData.track_authorization || "";
  if (transcodings.length === 0 || !trackAuth) {
    return { url: "", format: null, error: "no playable progressive transcoding" };
  }

  var transcoding = pickTranscoding(transcodings, preferFormat || "mp3");
  if (!transcoding) {
    return { url: "", format: null, error: "no compatible progressive transcoding" };
  }

  var lastError = "stream endpoint failed";
  for (var attempt = 0; attempt < 2; attempt++) {
    try {
      ensureClientId();
      var streamInfoURL = transcoding.url;
      var separator = streamInfoURL.indexOf("?") === -1 ? "?" : "&";
      streamInfoURL += separator + "client_id=" + state.clientId +
        "&track_authorization=" + encodeURIComponent(trackAuth);

      var response = http.get(streamInfoURL, {
        "User-Agent": utils.randomUserAgent(),
        "Accept": "application/json"
      });

      if (response && response.statusCode === 401 && attempt === 0) {
        state.clientId = null;
        state.clientIdExpiry = 0;
        continue;
      }
      ...
      var streamData = JSON.parse(response.body || "{}");
      if (streamData.url) {
        return { url: streamData.url, format: transcoding.format || null, error: "" };
      }
      lastError = "stream endpoint returned no URL";
    } catch (e) {
      lastError = e.message || String(e);
    }
  }

  return { url: "", format: transcoding.format || null, error: lastError };
}
...
function getTrack(trackId) {
  try {
    var trackData = fetchTrackData(trackId);
    var track = formatTrack(trackData);
    if (!track) return null;

    var stream = resolveProgressiveStreamURL(trackData, "mp3");
    if (stream.url) {
      track.preview_url = stream.url;
      log.info("[SC] Resolved playable stream for track:", trackId);
    } else {
      log.warn("[SC] Track metadata resolved without playable stream:", stream.error);
    }
    return track;
  } catch (e) {
...
function searchTracks(query, limit) {
  ...
  if (normalizedQuery === "trending" || normalizedQuery === "thịnh hành" || normalizedQuery === "popular") {
    var trending = fetchTrendingTracks(limit || 20);
