...
- GitHub Actions run `#9` on commit `1ee7fc6f551e3aabb06d2a85ad3e77527f4f3482` passed the mobile/desktop/Go/Rust/Tauri preflight and the unsigned iOS archive, bundle verification, package and upload. Artifact `OpenSpot-IPA` (ID `10300584963`) is 16,832,872 bytes with digest `sha256:074081856a35edca6eb72e4543704ec125d2012f2ba97546aa7f23f3d2f28ed3`; the extracted IPA contains app version `4.9.1` build `38` and `main.jsbundle`. Device regression coverage remains pending.
...
5. [x] Push the verified feature commit, dispatch the manual unsigned iOS IPA workflow once, and verify the uploaded artifact on the final commit; device regression coverage remains pending. Run `#9` is the current artifact evidence.
