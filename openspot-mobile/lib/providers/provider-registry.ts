...
export interface ProviderAdapter {
  ...
  searchTracks(query: string, page?: number, limit?: number): Promise<SearchResponse>;
...
class ProviderRegistryImpl {
  ...
  async getPriority(): Promise<ProviderId[]> {
    ...
    try {
      ...
      if (raw) {
        const parsed = parseStoredJSON<unknown>(raw, PROVIDER_PRIORITY_KEY, null);
  ...
  async searchTracks(query: string, page = 1, limit = 20, providerHint?: ProviderId | null): Promise<SearchResponse> {
    const providers = await this.orderedProviders(providerHint);
    let lastError: unknown = null;
    let emptyResult: SearchResponse | null = null;
    ...
    for (const provider of providers) {
      if (!provider.capabilities.has('search')) continue;
      try {
        const result = await provider.searchTracks(query, page, limit);
        if (!emptyResult) emptyResult = result;
  ...
  private async findEquivalentTrack(provider: ProviderAdapter, sourceTrack: Track): Promise<Track | null> {
    ...
    const response = await provider.searchTracks(query, 1, 10);
