...
interface ActivePlayer<T extends AutoMixTrack> {
  track: T;
...
export class AutoMixEngine<T extends AutoMixTrack> {
  ...
  setSettings(settings: AutoMixSettings): void {
    this.settings = settings;
    if (settings.mode === 'off') void this.cancelTransition(false);
  }

  setAlbumTrackIds(ids: Iterable<string>): void {
    this.albumTrackIds = new Set(ids);
  }
  ...
  async setPlaying(playing: boolean): Promise<void> {
    ...
    if (playing) {
      await this.current.player.play();
      this.setState('PLAYING');
    } else {
      await this.current.player.pause();
  ...
  plan(next: T | null | undefined): AutoMixTransitionPlan | null {
    ...
    return planAutoMixTransition(this.current.track, next, this.settings, this.albumTrackIds);
  }
  ...
  async maybeStartTransition(positionMs: number, durationMs: number, next: T | null | undefined): Promise<boolean> {
    ...
    this.transitionPromise = this.startTransition(next, plan).finally(() => {
  ...
  private async startTransition(next: T, plan: AutoMixTransitionPlan): Promise<void> {
    ...
    this.setState('PLAYING');
    this.emitDiagnostic({
      type: 'automix_started', trackId: next.id,
      detail: { durationMs: plan.durationMs, beatCount: plan.beatCount, bpmRatio: plan.bpmRatio, pitchRatio: plan.pitchRatio },
    });
    ...
    try {
      ...
      for (let step = 0; step <= AUTOMIX_STEP_COUNT; step += 1) {
        ...
        if (plan.bpmRatio !== 1 || plan.pitchRatio !== 1) {
          ...
          if (rate !== lastRate) {
            await outgoing.player.setRate(rate, true);
            lastRate = rate;
          }
          if (plan.pitchRatio !== 1 && pitch !== lastPitch) {
            await outgoing.player.setPitch?.(pitch);
            lastPitch = pitch;
          }
        }
      }
