...
import { createExpoAvAutoMixPlayer, configureAutoMixAudio } from './expo-av-auto-mix-player';
...
function toAutoMixTrack(track: Track | null): AutoMixTrack | null {
  ...
  return {
    ...
    audioMeta: track.audioMeta ?? null,
  };
}
...
export function useAutoMixPlayback(options: UseAutoMixPlaybackOptions): AutoMixPlaybackRuntime {
  ...
  if (!engineRef.current) {
    engineRef.current = new AutoMixEngine<AutoMixTrack>({
      createPlayer: async () => createExpoAvAutoMixPlayer(),
  ...
  useEffect(() => {
    ...
    void loadAutoMixSettings().then((loaded) => {
      ...
      if (loaded.mode !== 'off') void configureAutoMixAudio().catch((error) => recordDiagnostic({ category: 'automix', type: 'audio_mode_failed', data: { error: String(error) } }));
    });
    const unsubscribe = subscribeAutoMixSettings((next) => {
      setSettings(next);
      engine.setSettings(next);
      if (next.mode !== 'off') void configureAutoMixAudio().catch(() => {});
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      const player = await createExpoAvAutoMixPlayer();
      try {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (player) {
        ...
        if (typeof position === 'number' && typeof duration === 'number' && nextTrack) {
          await engine.maybeStartTransition(position, duration, toAutoMixTrack(nextTrack));
        }
      }
    };
  ...
  const seekTo = useCallback(async (value: number) => {
    const player = engine.getCurrentPlayer();
    await player?.seekToMs?.(Math.max(0, value));
  }, [engine]);
