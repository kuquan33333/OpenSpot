...
export interface HomeSectionDefinition { id: string; query: string; }
export interface ProviderHomeSection extends HomeSectionDefinition { providerId: ProviderId; tracks: Track[]; }
...
class ProviderRegistryImpl {
  ...
  async getHomeSections(definitions: HomeSectionDefinition[], limit = 12): Promise<ProviderHomeSection[]> {
    ...
    for (const definition of definitions) {
      for (const provider of providers) {
        try {
          const response = await provider.searchTracks(definition.query, 0, limit);
