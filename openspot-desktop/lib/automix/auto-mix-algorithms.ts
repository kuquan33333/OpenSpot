...
export function calculateKeyPitchRatio(current: SongAudioMeta, next: SongAudioMeta): number {
  ...
  for (const shift of [-1, 1, -2, 2]) {
    ...
    const shiftedCamelot: CamelotCode = {
      number: (currentCamelot.isMinor ? MINOR_CAMELOT_BY_PITCH : MAJOR_CAMELOT_BY_PITCH)[shiftedSemitone],
      isMinor: currentCamelot.isMinor,
    };
...
export function filterCutoffs(progress: number): FilterCutoffs {
  ...
  return {
    outgoingLowPassHz: exponentialInterpolate(LPF_START_HZ, LPF_END_HZ, filterProgress),
    incomingHighPassHz: exponentialInterpolate(HPF_START_HZ, HPF_END_HZ, filterProgress),
  };
}
...
export function resolveAutoCrossfadeDuration(current: SongAudioMeta | null | undefined, next: SongAudioMeta | null | undefined): { durationMs: number; beatCount: number | null } {
  ...
  const target = getAutoTargetDurationMs(currentBpm)
    * calculateBpmGapDurationFactor(currentBpm, nextBpm)
    * calculateKeyGapDurationFactor(current ?? {}, next ?? {});
  ...
  for (const beatCount of BEAT_COUNT_OPTIONS) {
    ...
    if (distance < bestDistance) {
      ...
      bestBeatCount = beatCount;
    }
  }
  return {
    durationMs: Math.min(AUTO_MAX_DURATION_MS, Math.max(AUTO_MIN_DURATION_MS, Math.trunc(bestBeatCount * beatMs))),
    beatCount: bestBeatCount,
  };
}
...
export function planAutoMixTransition(current: AutoMixTrack, next: AutoMixTrack | null | undefined, settings: AutoMixSettings, albumTrackIds: ReadonlySet<string> = new Set()): AutoMixTransitionPlan {
  const noTransition = (reason: string): AutoMixTransitionPlan => ({
    enabled: false,
    reason,
    durationMs: manualDuration(settings.durationMs),
    beatCount: null,
    bpmRatio: 1,
    pitchRatio: 1,
    currentCamelot: null,
  ...
  const bpmRatio = settings.mode === 'automix' && settings.bpmMatching && current.audioMeta?.bpm && next.audioMeta?.bpm
    ? calculateBpmSpeedRatio(current.audioMeta.bpm, next.audioMeta.bpm) : 1;
  const pitchRatio = settings.mode === 'automix' && settings.harmonicMatching
    ? calculateKeyPitchRatio(current.audioMeta ?? {}, next.audioMeta ?? {}) : 1;
  return {
    enabled: true,
    reason: 'ready',
    durationMs,
    beatCount: settings.durationMs === 'auto' ? autoDuration.beatCount : null,
    bpmRatio,
    pitchRatio,
    currentCamelot,
    nextCamelot,
...
export const DEFAULT_AUTO_MIX_SETTINGS: AutoMixSettings = {
  mode: 'off', durationMs: 'auto', djMode: false, bpmMatching: true,
  harmonicMatching: true, skipSameAlbum: false, showMeta: true,
};
