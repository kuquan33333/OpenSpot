...
func buildFilenameFromTemplate(template string, metadata map[string]any) string {
	...
	placeholders := map[string]string{
		"{title}": getString(metadata, "title"), "{artist}": getString(metadata, "artist"),
		"{album}": getString(metadata, "album"), "{track}": formatTrackNumber(getInt(metadata, "track")),
		"{track_raw}": formatRawNumber(getInt(metadata, "track")), "{playlist_position}": formatTrackNumber(getPlaylistPosition(metadata)),
		"{playlist position}": formatTrackNumber(getPlaylistPosition(metadata)), "{playlistPosition}": formatTrackNumber(getPlaylistPosition(metadata)),
		"{position}": formatTrackNumber(getPlaylistPosition(metadata)), "{playlist_position_raw}": formatRawNumber(getPlaylistPosition(metadata)),
		"{year}": yearValue, "{date}": dateValue, "{disc}": formatDiscNumber(getInt(metadata, "disc")),
		"{disc_raw}": formatRawNumber(getInt(metadata, "disc")), "{quality}": getString(metadata, "quality"),
		"{quality_variant}": getString(metadata, "quality_variant"), "{isrc}": getString(metadata, "isrc"),
		"{provider}": getString(metadata, "provider"), "{platform}": getString(metadata, "provider"),
		"{provider_id}": getString(metadata, "provider_id"), "{id}": getString(metadata, "provider_id"),
	}
...
func getDateValue(metadata map[string]any) string {
	if date := getString(metadata, "date"); date != "" {
		return date
	}
	if releaseDate := getString(metadata, "release_date"); releaseDate != "" {
		return releaseDate
	}
